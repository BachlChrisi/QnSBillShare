...
using QnSBillShare.Logic.Entities.Modules.BillExpense;
...
{
    partial class BillExpenses
    {
        internal Bill BillEntity { get; } = new Bill();
        ...
        public override int Id { get => BillEntity.Id; set => BillEntity.Id = value; }

        partial void OnNumberOfFriendsReading()
        {
            int result = 0;

            if (BillEntity != null && BillEntity.Friends.HasContent())
            {
                result = BillEntity.Friends.Split(";").Length;
            }
        ...
        partial void BeforeCopyProperties(IBillExpenses other, ref bool handled)
        {
        ...
        {
            return BillEntity != null && BillEntity.Friends.HasContent() ? BillEntity.Friends.Split(";") : new string[0];
        }
